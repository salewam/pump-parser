#!/usr/bin/env python3
"""
CDM/CDMF Pump Data Parser V19
==============================
+ Поддержка горизонтальных таблиц (CDM215, CDM200, CDM185)
+ Исправлен фильтр Q
"""
...
def parse_horizontal_table(lines: List[str], q_line_idx: int, q_values: List[float]) -> List[Dict]:
    """Парсинг горизонтальной таблицы (CDM185, CDM200, CDM215)"""
    pumps = []

    for i in range(q_line_idx + 1, len(lines)):
        line = lines[i].strip()
        if not line:
            continue
        if 'Таблица' in line or 'Серия' in line or 'CDM/' in line:
            break
        ...
        # Первая часть - ID модели
        model_part = parts[0]

        # Проверяем что это ID насоса (например 215-1-В, 185-1-А)
        if not re.match(r'\d{2,3}-', model_part):
            continue

        # Вторая часть - kW
        kw = parse_num(parts[1])
        if not kw or kw < 0.1 or kw > 250:
            continue

        # Остальные части - H значения
        h_values = []
        for p in parts[2:]:
            h = parse_num(p)
        ...
        # Формируем model_id
        model_id = 'CDM' + model_part.replace('−', '-').replace('З', '3')
...
def extract_cdm_from_pdf(pdf_path: str) -> List[Dict]:
    ...
    with fitz.open(pdf_path) as doc:
        for page in doc:
            ...
            # === ГОРИЗОНТАЛЬНЫЙ ФОРМАТ (CDM185, CDM200, CDM215) ===
            # Q (м3/ч)  0  100  120  140  160  180  200  215  220  240  260  280
            horizontal_q = []
            q_line_idx = -1
            for i, line in enumerate(lines):
                if 'Q' in line and ('м3/ч' in line or 'м³/ч' in line):
                    # Парсим Q значения из этой же строки
                    parts = line.split()
                    for p in parts:
                        q = parse_num(p)
                        if q is not None and 0 <= q <= 500 and q not in GARBAGE_Q:
                            horizontal_q.append(q)
                    if len(horizontal_q) >= 5:
                        q_line_idx = i
                        break
                    horizontal_q = []

            if horizontal_q and q_line_idx >= 0:
                h_pumps = parse_horizontal_table(lines, q_line_idx, horizontal_q)
                if h_pumps:
                    for p in h_pumps:
...
def main():
    if len(sys.argv) < 2:
        print("Использование: python3 cdm_parser_v19.py <input.pdf> [output.json]")
        sys.exit(1)
    ...
    print(f"V19: {len(pumps)} записей, {len(models)} моделей")
