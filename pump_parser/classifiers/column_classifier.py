...
log = logging.getLogger("pump_parser.column_classifier")
...
def _match_header_keywords(header: str) -> Optional[Tuple[str, int]]:
    ...
    for field, info in COLUMN_PATTERNS.items():
        ...
        # Use [^a-zA-Z] instead of \b to handle Latin→Cyrillic boundaries (Qном, Hном, P2)
        symbol = info.get("symbol", "")
        if symbol and re.match(rf'^{re.escape(symbol)}(?:[^a-zA-Z]|$)', h_clean, re.I):
            score = 5
            if score > best_score:
                best_score = score
                best_field = field
