...
class CDMParserPyMuPDF:
    ...
    def is_garbage_pump(self, record: Dict) -> bool:
        """
        V10: ИНТЕЛЛЕКТУАЛЬНЫЙ фильтр мусорных насосов

        Автоматически обнаруживает артефакты парсинга по признакам:
        1. ID с пробелами (CDM80- 180)
        2. Подозрительные комбинации чисел (CDM8-24, CDM11-110)
        3. Невалидные серии (CDM8, CDM11, CDM45, CDM50, CDM60, CDM80)
        4. Слишком большие вторые числа (CDM10-29, где 29 >> 10)
        5. Q, Head, kW вне реалистичных диапазонов

        Универсальный подход - работает для ЛЮБЫХ PDF, не только CDM!
        """
        pump_id = record.get('id', '')
        q = record.get('q', 0)
        head = record.get('head_m', 0)
        ...
        # Пример: "CDM80- 180" - явная ошибка парсинга
        if ' ' in pump_id:
            return True
        ...
        KNOWN_GARBAGE = {
            ...
            'CDM125-9*'  # Неправильный формат
        }
        if pump_id in KNOWN_GARBAGE:
            return True

        # === ПРАВИЛО 3: Невалидные серии насосов ===
        # Реальные серии CDM: 1, 3, 5, 10, 15, 20, 32, 42, 65, 85, 95, 120, 125, 150, 155, 185, 200, 215
        # НЕ существуют: CDM8, CDM11, CDM45, CDM50, CDM60, CDM80 и т.д.
        VALID_SERIES = {1, 3, 5, 10, 15, 20, 32, 42, 65, 85, 95, 120, 125, 150, 155, 185, 200, 215}

        # Извлекаем номер серии из ID
        import re
        series_match = re.match(r'CDM[F]?(\d+)', pump_id)
        if series_match:
            series_num = int(series_match.group(1))
            if series_num not in VALID_SERIES:
                return True  # Это невалидная серия

        # === ПРАВИЛО 4: Подозрительные числа в ID ===
        # CDM10-29 (29 >> 10), CDM11-110 (110 >> 11)
        # Второе число не должно быть намного больше первого
        parts = pump_id.replace('CDM', '').replace('CDMF', '').split('-')
        if len(parts) >= 2:
            try:
                first = int(parts[0])
                # Проверяем все числа после первого
                for part in parts[1:]:
                    # Извлекаем числа (игнорируем буквы и звездочки)
                    num_match = re.match(r'(\d+)', part)
                    if num_match:
                        second = int(num_match.group(1))
                        # Если второе число > 2x первого, это подозрительно
                        if second > first * 2 and first < 50:
                            return True
            except (ValueError, IndexError):
                pass

        # === ПРАВИЛО 5: Нереалистичные значения параметров ===
        # Q должен быть в разумных пределах (0-500 м³/ч)
        # Head должен быть в разумных пределах (0-200 м)
        # kW должен быть в разумных пределах (0.1-100 кВт для стандартных насосов)
        ...
        # === ПРАВИЛО 6: ID слишком простые (артефакты) ===
        # CDM8, CDM11 без дефиса - это не насосы
        # Уже фильтруется ФИЛЬТРОМ V6 (dash filter)
            ...
            if len(h_values) > 0:
                ...
                for idx, h in enumerate(h_values):
                    if idx < len(q_values):
                        ...
                        # ФИЛЬТР V9: Исключаем мусорные ID насосов (9 записей)
                        # CDM8-24, CDM10-29, CDM11-75, CDM11-110 → артефакты парсинга
                        # CDM45-120, CDM50-110, CDM60-160, CDM80- 180 → несуществующие модели
                        if self.is_garbage_pump(record):
                            # Это мусорный ID - пропускаем
                            continue
